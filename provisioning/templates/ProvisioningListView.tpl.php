<?php
	$this->assign('title','SPOT | Provisionings');
	$this->assign('nav','provisionings');

	$this->display('_Header.tpl.php');
?>

<script type="text/javascript">
	$LAB.script("scripts/app/provisionings.js").wait(function(){
...
<h1>
	<i class="icon-th-list"></i> Provisionings
	...
	<script type="text/template" id="provisioningCollectionTemplate">
		<table class="collection table table-bordered table-hover">
		<thead>
			<tr>
				<th id="header_Provisioningid">Provisioningid<% if (page.orderBy == 'Provisioningid') { %> <i class='icon-arrow-<%= page.orderDesc ? 'up' : 'down' %>' /><% } %></th>
...
<!-- UNCOMMENT TO SHOW ADDITIONAL COLUMNS
				...
				<th id="header_Logtime">Logtime<% if (page.orderBy == 'Logtime') { %> <i class='icon-arrow-<%= page.orderDesc ? 'up' : 'down' %>' /><% } %></th>
				...
				<th id="header_Remotecommandid">Remotecommandid<% if (page.orderBy == 'Remotecommandid') { %> <i class='icon-arrow-<%= page.orderDesc ? 'up' : 'down' %>' /><% } %></th>
...
-->
		...
		<% items.each(function(item) { %>
			<tr id="<%= _.escape(item.get('provisioningid')) %>">
				<td><%= _.escape(item.get('provisioningid') || '') %></td>
...
<!-- UNCOMMENT TO SHOW ADDITIONAL COLUMNS
				...
				<td><%= _.escape(item.get('logtime') || '') %></td>
...
-->
	...
	<script type="text/template" id="provisioningModelTemplate">
		<form class="form-horizontal" onsubmit="return false;">
			<fieldset>
				<div id="provisioningidInputContainer" class="control-group">
					<label class="control-label" for="provisioningid">Provisioningid</label>
					<div class="controls inline-inputs">
						<input type="text" class="input-xlarge" id="provisioningid" placeholder="Provisioningid" value="<%= _.escape(item.get('provisioningid') || '') %>">
				...
				<div id="logtimeInputContainer" class="control-group">
					<label class="control-label" for="logtime">Logtime</label>
					<div class="controls inline-inputs">
						<input type="text" class="input-xlarge" id="logtime" placeholder="Logtime" value="<%= _.escape(item.get('logtime') || '') %>">
						<span class="help-inline"></span>
					</div>
				</div>
				...
				<div id="remotecommandidInputContainer" class="control-group">
					<label class="control-label" for="remotecommandid">Remotecommandid</label>
					<div class="controls inline-inputs">
						<input type="text" class="input-xlarge" id="remotecommandid" placeholder="Remotecommandid" value="<%= _.escape(item.get('remotecommandid') || '') %>">
						<span class="help-inline"></span>
					</div>
				</div>
		...
		<form id="deleteProvisioningButtonContainer" class="form-horizontal" onsubmit="return false;">
			<fieldset>
				<div class="control-group">
					<label class="control-label"></label>
					<div class="controls">
						<button id="deleteProvisioningButton" class="btn btn-mini btn-danger"><i class="icon-trash icon-white"></i> Delete Provisioning</button>
						<span id="confirmDeleteProvisioningContainer" class="hide">
							<button id="cancelDeleteProvisioningButton" class="btn btn-mini">Cancel</button>
							<button id="confirmDeleteProvisioningButton" class="btn btn-mini btn-danger">Confirm</button>
						</span>
					</div>
				</div>
			</fieldset>
	...
	<!-- modal edit dialog -->
	<div class="modal hide fade" id="provisioningDetailDialog">
		<div class="modal-header">
			<a class="close" data-dismiss="modal">&times;</a>
			<h3>
				<i class="icon-edit"></i> Edit Provisioning
				<span id="modelLoader" class="loader progress progress-striped active"><span class="bar"></span></span>
			</h3>
		</div>
		<div class="modal-body">
			<div id="modelAlert"></div>
			<div id="provisioningModelContainer"></div>
		</div>
		<div class="modal-footer">
			<button class="btn" data-dismiss="modal" >Cancel</button>
			<button id="saveProvisioningButton" class="btn btn-primary">Save Changes</button>
		</div>
	</div>

	<div id="collectionAlert"></div>
	
	<div id="provisioningCollectionContainer" class="collectionContainer">
	</div>

	<p id="newButtonContainer" class="buttonContainer">
		<button id="newProvisioningButton" class="btn btn-primary">Add Provisioning</button>
	</p>
