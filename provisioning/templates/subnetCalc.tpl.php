<?php
$this->assign('title', 'SPOT | Subnet Calculator');
$this->assign('nav', 'subnetcalculator');
...
<!--    <span id=loader class="loader progress progress-striped active"><span class="bar"></span></span> -->
    ...
    <form method="post" action="<?php echo GlobalConfig::$ROOT_URL . $this->nav; ?>">
        <table class=" table-bordered table-responsive table table-striped">
            <tr>
                <th>IP &amp; Mask or CIDR:</th>
                <td><input type="text" name="my_net_info" value="" size="31" maxlength="32"></td>
                <td><input type="submit" value="Calculate" name="subnetcalc" class="btn btn-primary" />
                </td>
            </tr>
        </table>
    </form>
    ...
    <?php
    echo "<table class='stselection table-bordered table-responsive table table-striped'>";
...
// Print Results
        tr('Address:', "<font color=\"blue\">$dq_host</font>", '<font color="brown">' . dotbin($bin_host, $cdr_nmask) . '</font>');
        tr('Netmask:', '<font color="blue">' . bintodq($bin_nmask) . " = $cdr_nmask</font>", '<font color="red">' . dotbin($bin_nmask, $cdr_nmask) . '</font>');
        tr('Wildcard:', '<font color="blue">' . bintodq($bin_wmask) . '</font>', '<font color="brown">' . dotbin($bin_wmask, $cdr_nmask) . '</font>');
        tr('Network:', '<font color="blue">' . bintodq($bin_net) . '</font>', "<font color=\"brown\">$dotbin_net</font><span class='pull-right'><font color=\"Green\">(Class $class)</font></span>");
        tr('Broadcast:', '<font color="blue">' . bintodq($bin_bcast) . '</font>', '<font color="brown">' . dotbin($bin_bcast, $cdr_nmask) . '</font>');
        tr('HostMin:', '<font color="blue">' . bintodq($bin_first) . '</font>', '<font color="brown">' . dotbin($bin_first, $cdr_nmask) . '</font>');
        tr('HostMax:', '<font color="blue">' . bintodq($bin_last) . '</font>', '<font color="brown">' . dotbin($bin_last, $cdr_nmask) . '</font>');
