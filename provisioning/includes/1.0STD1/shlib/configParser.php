<?
   ...
   ********************************************************************/
   ...
   class CONFIGPARSER
   {
      ...
      public function __construct($pathToConfig="../param/productiondb.cfg")
      { 
     
        if ( $pathToConfig[0] != '/' )    // Test if the path is absolute, and if no
          $pathToConfig = getcwd().'/'.$pathToConfig; // append the current folder to the root
        
		if ( $pathToConfig == "" or  ! is_readable( $pathToConfig ))            ///Test if the file is correct
          throw new Exception(EXCEPTION_FILE_ERROR,$code=EXCEPTION_FILE_ERROR_EC);
