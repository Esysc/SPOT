...
class LDAPAuth
{
	...
	public function connect()
	{
		...
		{
			$this->_conn = ldap_connect($dc, 3268);
		}
	...
	public function return_email($username, $password, $search_dn )
	{
		if (($username === NULL) || ($password === NULL) || empty($username) || empty($password))
		{
			return false;
		...
		$this->_data = @ldap_search($this->_conn, $search_dn, "(sAMAccountName=".$username.")");
		$entries = ldap_get_entries($this->_conn, $this->_data);
