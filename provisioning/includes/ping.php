...
$hostarr = json_decode($hosts, true);
...
foreach ($hostarr as $host) {
    ...
    $latency = $ping->ping();
    if ($latency !== false) {
        $reponse[$host] = "<div data-toggle='tooltip' title='$hostname is Alive' class='label label-success'>$hostname is Alive</div>";
    } else {
        // $reponse[$host] = "<div data-toggle='tooltip' title='$hostname could not be reached.' class='label label-warning'>$hostname could not be reached.</div><br />";
        $reponse[$host] = "<div data-toggle='tooltip' title='$hostname is down!.' class='label label-warning'><strong>  $hostname is down!</strong></div>
                          <script>
                          $('#servermsg').html('Attention required!<strong>  $hostname is down!</strong>');
                          $('#basicModal').modal();
                          </script>";
    }
}
