...
$objPHPExcel->getProperties()->setCreator('SysProd')
        ...
        ->setTitle($title)
        ->setSubject('Excel export of ' . $title . ' ' . $filename)
...
for ($z = 0; $z <= $tbcnt; $z++) {
    ...
    $worksheet->setCellValue('O1', 'Customers');
    $Orow = 2;
    foreach ($cust_array_def as $key => $customer) {
        $worksheet->setCellValue('O' . $Orow, $customer);
        $Orow++;
    }
    ...
    for ($b = 0; $b < count($bodyrows); $b++) {
        ...
        for ($t = 0; $t < count($td); $t++) {
            ...
            $style_overlay['fill']['color']['rgb'] = $thisbg;
            if ($thiscolspan == 1) {
                $worksheet->getColumnDimension($xcol)->setWidth(23);
            }
            /* begin of switch */
            ...
            switch ($xcol) {
                case 'B':
                    /*   if (!empty($thistext)) {
                      ...
                      { */
                    $objValidation = $objPHPExcel->getActiveSheet()->getCell($xcol . $xrow)->getDataValidation();
                    ...
                    $objValidation->setFormula1('=$O$2:$O$' . $Orow);
                    $objPHPExcel->getActiveSheet()->getCell($xcol . $xrow)->setDataValidation($objValidation);
                    $worksheet->setCellValue($xcol . $xrow, $thistext);
                    break;
                case 'C':
                    $objValidation = $objPHPExcel->getActiveSheet()->getCell($xcol . $xrow)->getDataValidation();
                    ...
                    $objValidation->setPromptTitle('Pick from list');
                    $objValidation->setPrompt('Please pick a value from the drop-down list.');
                    $objValidation->setFormula1('"APAC,China,NCE,NMR,SMEA"');
                    $objPHPExcel->getActiveSheet()->getCell($xcol . $xrow)->setDataValidation($objValidation);
                    $worksheet->setCellValue($xcol . $xrow, $thistext);
                    break;
                case 'E':
                    ...
                    $objValidation->setPromptTitle('Pick from list');
                    $objValidation->setPrompt('Please pick a value from the drop-down list.');
                    $objValidation->setFormula1('"IDTV,Mediaguard,Mycompany,Quative"');
                    $objPHPExcel->getActiveSheet()->getCell($xcol . $xrow)->setDataValidation($objValidation);
                    $worksheet->setCellValue($xcol . $xrow, $thistext);
                    break;
                case 'F':
                    ...
                    $objValidation->setPromptTitle('Pick from list');
                    $objValidation->setPrompt('Please pick a value from the drop-down list.');
                    $objValidation->setFormula1('"Database,Firewall,GUI,Network Equipment,Remote Access,Subnet,Unix,VPN ISDN Secret,Windows"');
                    $objPHPExcel->getActiveSheet()->getCell($xcol . $xrow)->setDataValidation($objValidation);
                    $worksheet->setCellValue($xcol . $xrow, $thistext);
                    break;
                case 'N':
                    ...
                    $objValidation->setPromptTitle('Pick from list');
                    $objValidation->setPrompt('Please pick a value from the drop-down list.');
                    $objValidation->setFormula1('"Analog,ISDN,VPN"');
                    $objPHPExcel->getActiveSheet()->getCell($xcol . $xrow)->setDataValidation($objValidation);
                    $worksheet->setCellValue($xcol . $xrow, $thistext);
                    break;
...
saveExcelToLocalFile($objWriter, $filename);
