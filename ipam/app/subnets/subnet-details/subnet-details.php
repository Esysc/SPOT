...
<?php } ?>
    ...
    <tr>
        ...
        <td>
<?php
if (empty($vlan['number']) || $vlan['number'] == 0) {
    $vlan['number'] = "<span class='text-muted'>/</span>";
} //Display fix for emprt VLAN
print $vlan['number'];

if (!empty($vlan['name'])) {
    print ' - ' . $vlan['name'];
}     //Print name if provided
if (!empty($vlan['description'])) {
    print ' [' . $vlan['description'] . ']';
}  //Print description if provided
// domain
$l2domain = $Tools->fetch_object("vlanDomains", "id", $vlan['domainId']);
if ($l2domain !== false) {
    print " <span class='badge badge1 badge5' rel='tooltip' title='VLAN is in domain $l2domain->name'>$l2domain->name " . _('Domain') . " </span>";
}
?>
        </td>
    </tr>
...
<?php } ?>
    ...
    if (!$slaves) {
        ...
        if ($User->settings->enableIPrequests == 1) {
            ...
            if (@$subnet['isFull'] == 1) {
                print "	<td>" . _('disabled - marked as full') . "</td>";
            }  # yes
            elseif ($subnet['allowRequests'] == 1) {
                print "	<td><span class='badge badge1 badge5 alert-success'>" . _('enabled') . "</span></td>";
            }  # yes
            else {
                print "	<td><span class='badge badge1 badge5'>" . _('disabled') . "</span></td>";
            }  # no
            print "</tr>";
        }
    ...
    if ($User->settings->enablePowerDNS == 1) {
        ...
        if ($subnet['DNSrecursive'] == 1) {
            print "	<td><span class='badge badge1 badge5 alert-success'>" . _('enabled') . "</span> $btns $zone</td>";
        }  # yes
        else {
            print "	<td><span class='badge badge1 badge5'>" . _('disabled') . "</span></td>";
        }  # no
    ...
    if (!$slaves) {
        ...
        print "<a class='hostExport btn btn-xs btn-default'  href='' data-container='body' rel='tooltip' title='" . _('Export IP addresses for hosts file') . "' data-subnetId='$subnet[id]'>		<i class='fa fa-upload'></i></a>";
        //share
