...
<div class="btn-group">
    <?php if ($_GET['page'] == "administration") { ?>
        ...
        if ($User->is_admin(false)) {
            ...
            ?>
            <button class='btn btn-sm btn-default' style='margin-bottom:10px;' id="manageLocations"><i class='fa fa-pencil'></i> <?php print _('Manage'); ?></button>
            <?php
        }
    }
    ?>
                ...
                foreach ($custom as $field) {
                    if (!in_array($field['name'], $hidden_custom_fields)) {
                        ...
                        if ($field['type'] == "tinyint(1)") {
                            if ($l->$field['name'] == "0") {
                                print _("No");
                            } elseif ($l->$field['name'] == "1") {
                                print _("Yes");
                            }
                        }
                        //text
                        elseif ($field['type'] == "text") {
                            if (strlen($l->$field['name']) > 0) {
