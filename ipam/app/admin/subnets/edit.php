...
if ($_POST['action'] != "add") {
    ...
    # false id
    if (sizeof($subnet_old_details) == 0) {
        $Result->show("danger", _("Invalid subnetId"), true, true);
    }
}
...
// change - agent selector
        $('.input-switch-agents-ping, .input-switch-agents-scan').on('switchChange.bootstrapSwitch', function (e, data) {
            ...
            // change
            if (ping == true || scan == true) {
                $("tr#scanAgentDropdown").removeClass("hidden");
            }
            else if (ping == false && scan == false) {
                $("tr#scanAgentDropdown").addClass("hidden");
            }
            $('select').chosen();
        });
    ...
    <form id="editSubnetDetails">
        <table class="editSubnetDetails table table-bordered table-condensed ">
            ...
            <tr>
                ...
                <td>
                    <?php
                    if ($_POST['subnetId'] && $_POST['action'] == "add") {
                        $showDropMenuFull = 1;
                    }
                    # set CIDR
...
// loop
            ...
            /* set default value */
            if (empty($subnet_old_details['allowRequests'])) {
                $subnet_old_details['allowRequests'] = "0";
            }
                ...
                if ($vrfs != false) {
                    foreach ($vrfs as $vrf) {
                        ...
                        if (strlen($vrf->sections) == 0 || in_array(@$_POST['sectionId'], $permitted_sections)) {
                            ...
                            if ($vrf['vrfId'] == $subnet_old_details['vrfId']) {
                                print '<option value="' . $vrf['vrfId'] . '" selected>' . $vrf['name'] . $vrf['description'] . '</option>';
                            } else {
            ...
            // set hidden
            if (@$subnet_old_details['pingSubnet'] == "1" || @$subnet_old_details['discoverSubnet'] == "1") {
                $hidden = "";
            } else {
                $hidden = "hidden";
            }
            ...
            if ($agents !== false) {
                ...
                foreach ($agents as $a) {
                    if ($a->id == @$subnet_old_details['scanAgent']) {
                        print "<option value='" . $a->id . "' selected='selected'>" . $a->name . " (" . $a->description . ")</option>";
                    } else {
                        print "<option value='" . $a->id . "'>" . $a->name . " (" . $a->description . ")</option>";
                    }
                }
                print "</select>";
                print "</td>";
...
//show names instead of ip address
            ...
            if (@$subnet_old_details['showName'] == 1) {
                print 'checked';
            }
            print ' >' . "\n";
...
//hidden ones
            ...
            if (sizeof($custom_fields) > 0) {
                ...
                foreach ($custom_fields as $field) {
                    ...
                    # set default value !
                    if ($_POST['action'] == "add") {
                        $subnet_old_details[$field['name']] = $field['Default'];
                    }
                    ...
                    if (substr($field['type'], 0, 3) == "set" || substr($field['type'], 0, 4) == "enum") {
                        ...
                        foreach ($tmp as $v) {
                            if ($v == $subnet_old_details[$field['name']]) {
                    ...
                    elseif ($field['type'] == "date" || $field['type'] == "datetime") {
                        ...
                        //field
                        if (!isset($subnet_old_details[$field['name']])) {
                            print ' <input type="text" class="' . $class . ' form-control input-sm input-w-auto" data-format="' . $format . '" name="' . $field['nameNew'] . '" maxlength="' . $size . '" rel="tooltip" data-placement="right" title="' . $field['Comment'] . '">' . "\n";
                        } else {
                            print ' <input type="text" class="' . $class . ' form-control input-sm input-w-auto" data-format="' . $format . '" name="' . $field['nameNew'] . '" maxlength="' . $size . '" value="' . $subnet_old_details[$field['name']] . '" rel="tooltip" data-placement="right" title="' . $field['Comment'] . '">' . "\n";
                        }
                    }
                    //boolean
                    elseif ($field['type'] == "tinyint(1)") {
                        ...
                        foreach ($tmp as $k => $v) {
                            if (strlen($subnet_old_details[$field['name']]) == 0 && $k == 2) {
                                print "<option value='$k' selected='selected'>" . _($v) . "</option>";
                            } elseif ($k == $subnet_old_details[$field['name']]) {
                                print "<option value='$k' selected='selected'>" . _($v) . "</option>";
                            } else {
