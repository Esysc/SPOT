        ...
        <table id="editLocation" class="table table-noborder table-condensed">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?php print _('<button class="btn btn-default btn-success" id="geocode" data-attribute="' . $location->address . '">Get lat and long from address</button>'); ?></td>
                </tr>
                ...
                if (sizeof($custom) > 0) {
                    ...
                    foreach ($custom as $field) {
                        ...
                        print '<tr>' . "\n";
                        print '	<th>' . ucwords($field['name']) . ' ' . $required . '</th>' . "\n";
                        print '	<td>' . "\n";
                        ...
                        elseif ($field['type'] == "date" || $field['type'] == "datetime") {
                            ...
                            if (!isset($location->$name)) {
                                ...
                                print ' <input type="text" class="' . $class . ' form-control input-sm input-w-auto" data-format="' . $format . '" name="' . $field['nameNew'] . '" maxlength="' . $size . '" value="' . $location->$name . '" rel="tooltip" data-placement="right" title="' . $field['Comment'] . '">' . "\n";
                            }
                        }
                        ...
                        elseif ($field['type'] == "tinyint(1)") {
                            ...
                            foreach ($tmp as $k => $v) {
                                if (strlen($location->$name) == 0 && $k == 2) {
                                    print "<option value='$k' selected='selected'>" . _($v) . "</option>";
                                } elseif ($k == $location->$name) {
                                    print "<option value='$k' selected='selected'>" . _($v) . "</option>";
                                } else {
                        ...
                        elseif ($field['type'] == "text") {
                            print ' <textarea class="form-control input-sm" name="' . $field['nameNew'] . '" placeholder="' . $field['name'] . '" rowspan=3 rel="tooltip" data-placement="right" title="' . $field['Comment'] . '">' . $location->$name . '</textarea>' . "\n";
                        }
                        //default - input field
                        else {
                            if ($field['nameNew'] === "User") {
                                ...
                                print ' <input type="text" class="ip_addr form-control input-sm" name="' . $field['nameNew'] . '" placeholder="' . $field['name'] . '" value="' . $location->$name . '" size="30" rel="tooltip" data-placement="right" title="' . $field['Comment'] . '">' . "\n";
                            }
                        }
                        print '	</td>' . "\n";
                        print '</tr>' . "\n";
...
<div class="pFooter">
    <div class="btn-group">
        ...
        ?>" id="editLocationSubmit"><i class="fa <?php
                if ($_POST['action'] == "add") {
                    print "fa-plus";
