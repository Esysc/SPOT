...
class Addresses_controller extends Common_api_functions {
    ...
    public function POST() {
        ...
        if ($this->_params->id == "first_free") {
            ...
            if ($subnet === false) {
                $this->Response->throw_exception(404, "Invalid subnet identifier");
            }
            if ($subnet->isFull == 1) {
                $this->Response->throw_exception(404, "No free addresses found");
            }
            ...
            if ($this->_params->ip_addr == false) {
                ...
                if (!isset($this->_params->description))
                    $this->_params->description = "API created";
            }
        }
    ...
    public function PATCH() {
        ...
        if (isset($this->_params->subnetId)) {
            $this->Response->throw_exception(400, "Subnet cannot be changed");
        }
    ...
    private function validate_address_id() {
        if (!$this->old_address = $this->Addresses->fetch_address("id", $this->_params->id)) {
            $this->Response->throw_exception(404, "Address does not exist");
        }
    }
    ...
    private function validate_tag() {
        // numeric
        if (!is_numeric(@$this->_params->id2)) {
            $this->Response->throw_exception(400, 'Invalid tag identifier');
        }
        // check db
        if (!$this->Tools->fetch_object("ipTags", "id", $this->_params->id2)) {
            $this->Response->throw_exception(404, "Address tag does not exist");
        }
    }
    ...
    private function validate_subnet() {
        // numberic
        if (!is_numeric($this->_params->subnetId)) {
            $this->Response->throw_exception(400, "Subnet Id must be numeric");
        }
        // check subnet
        if (is_null($res = $this->Subnets->fetch_subnet("id", $this->_params->subnetId))) {
    ...
    private function validate_create_parameters() {
        ...
        if ($this->Addresses->address_exists($this->_params->ip_addr, $this->_params->subnetId)) {
            $this->Response->throw_exception(400, "IP address already exists");
        }
        ...
        if (isset($this->_params->switch)) {
            if ($this->Tools->fetch_object("devices", "vlanId", $this->_params->switch) === false) {
