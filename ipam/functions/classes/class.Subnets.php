...
class Subnets extends Common_functions {
    ...
    public function modify_subnet($action, $values) {
        ...
        if ($action == "add") {
            ...
            return $this->subnet_delete($values['id']);
        } elseif ($action == "truncate") {
            return $this->subnet_truncate($values['id']);
        } elseif ($action == "resize") {
            return $this->subnet_resize($values['id'], $values['subnet'], $values['mask']);
        } else {
    ...
    private function subnet_add($values) {
        ...
        # execute
        try {
            $this->Database->insertObject("subnets", $values);
        } catch (Exception $e) {
    ...
    private function subnet_edit($values) {
        ...
        # execute
        try {
            $this->Database->updateObject("subnets", $values, "id");
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage(), false);
    ...
    public function subnet_truncate($subnetId) {
        ...
        # execute
        try {
            $this->Database->deleteRow("ipaddresses", "subnetId", $subnetId);
        } catch (Exception $e) {
    ...
    private function subnet_resize($subnetId, $subnet, $mask) {
        ...
        # execute
        try {
            $this->Database->updateObject("subnets", array("id" => $subnetId, "subnet" => $subnet, "mask" => $mask), "id");
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage(), false);
    ...
    public function subnet_split($subnet_old, $number, $prefix, $group = "yes", $strict = "yes") {
        ...
        foreach ($newsubnets as $subnet) {
            ...
            if (isset($ids)) {
                if (!$Admin->object_modify("ipaddresses", "edit-multiple", $ids, array("subnetId" => $this->lastInsertId))) {
                    $this->Result->show("danger", _("Failed to move IP address"), true);
                }
            }
        ...
        if ($group != "yes") {
            if (!$Admin->object_modify("subnets", "delete", "id", array("id" => $subnet_old->id))) {
                $this->Result->show("danger", _("Failed to remove old subnet"), true);
            }
        }
    ...
    public function fetch_section_subnets($sectionId) {
        ...
        # fetch
        try {
            $subnets = $this->Database->getObjectsQuery("SELECT *,LPAD(subnet, 32, 0) as `subnet_int` FROM `subnets` where `sectionId` = ? order by `isFolder` desc, case `isFolder` when 1 then description else $order[0] end $order[1]", array($sectionId));
        } catch (Exception $e) {
    ...
    public function fetch_all_subnets_search($type = "IPv4") {
        ...
        if ($type == "IPv4") {
            $query = "SELECT `id`,`subnet`,`mask` FROM `subnets` where `subnet` < 4294967295;";
        } else {
            $query = "SELECT `id`,`subnet`,`mask` FROM `subnets` where `subnet` > 4294967295;";
        }
        # fetch
    ...
    public function fetch_all_subnets_for_pingCheck($agentId = null) {
        ...
        # fetch
        try {
            $subnets = $this->Database->getObjectsQuery("SELECT `id`,`subnet`,`sectionId`,`mask` FROM `subnets` where `scanAgent` = ? and `pingSubnet` = 1 and `isFolder`= 0 and `mask` > '0' and subnet > 16843009;", array($agentId));
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    public function fetch_all_subnets_for_discoveryCheck($agentId = null) {
        ...
        # fetch
        try {
            $subnets = $this->Database->getObjectsQuery("SELECT `id`,`subnet`,`sectionId`,`mask` FROM `subnets` where `scanAgent` = ? and `discoverSubnet` = 1 and `isFolder`= 0 and `isFull`!= 1 and `mask` > '0' and subnet > 16843009 and `mask` > 20;", array($agentId));
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    public function fetch_vlan_subnets($vlanId, $sectionId = null) {
        ...
        if (@$section->subnetOrdering != "default" && strlen(@$section->subnetOrdering) > 0) {
            $order = explode(",", $section->subnetOrdering);
        } else {
            $order = $this->get_subnet_order();
        }
        ...
        # fetch
        try {
            $subnets = $this->Database->getObjectsQuery($query, $params);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    public function is_linked($subnetId) {
        if (!is_numeric($subnetId)) {
            ...
            try {
                $subnets = $this->Database->getObjectsQuery("select * from subnets where `linked_subnet` = ?", array($subnetId));
            } catch (Exception $e) {
                $this->Result->show("danger", _("Error: ") . $e->getMessage());
                return false;
            }
    ...
    public function fetch_scanning_agents() {
        # fetch
        try {
            $agents = $this->Database->getObjects("scanAgents");
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    public function fetch_scanned_subnets($agentId = null) {
        ...
        # fetch
        try {
            $subnets = $this->Database->getObjectsQuery($query, array($agentId));
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    public function find_gateway($subnetId) {
        ...
        # fetch
        try {
            $gateway = $this->Database->getObjectQuery($query, array($subnetId));
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    public function find_subnet($sectionId = false, $cidr = false) {
        ...
        if ($this->verify_cidr_address($cidr) !== true) {
            return false;
        }
        // set subnet / mask
        $tmp = explode("/", $cidr);
        // search
        try {
            $subnet = $this->Database->getObjectQuery("select * from `subnets` where `subnet`=? and `mask`=? and `sectionId`=?;", $values = array($this->transform_address($tmp[0], "decimal"), $tmp[1], $sectionId));
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    public function get_max_hosts($netmask, $ipversion, $strict = true) {
        if ($ipversion == "IPv4") {
            return $this->get_max_IPv4_hosts($netmask, $strict);
        } else {
            return $this->get_max_IPv6_hosts($netmask);
        }
    }
    ...
    public function get_network_boundaries($address, $netmask) {
        ...
        # return boundaries
        if ($ipversion == "IPv4") {
            return $this->get_IPv4_network_boundaries($address, $netmask);
        } else {
            return $this->get_IPv6_network_boundaries($address, $netmask);
        }
    }
    ...
    public function verify_cidr_address($cidr, $issubnet = true) {
        ...
        if (strlen($error = $this->verify_cidr($cidr)) > 0) {
            return $error;
        }
        # make checks
    ...
    public function verify_cidr_address_IPv4($cidr, $issubnet = true) {
        ...
        if ($net = $this->Net_IPv4->parseAddress($cidr)) {
            if (!$this->Net_IPv4->validateIP($net->ip)) {
                return _("Invalid IP address!");
            }              //validate IP
            elseif (($net->network != $net->ip) && ($issubnet)) {
                return _("IP address cannot be subnet! (Consider using") . " " . $net->network . ")";
            }  //network must be same as provided IP address
            elseif (!$this->Net_IPv4->validateNetmask($net->netmask)) {
                return _('Invalid netmask') . ' ' . $net->netmask;
            }             //validate netmask
            else {
                return true;
            }
        } else {
            return _('Invalid CIDR format!');
        }
    }
    ...
    public function verify_cidr_address_IPv6($cidr, $issubnet = true) {
        ...
        $this->initialize_pear_net_IPv6();
        # validate
        if (!$this->Net_IPv6->checkIPv6($cidr)) {
            ...
            if (($subnetParse[0] != $subnet) && ($issubnet)) {
                return _("IP address cannot be subnet! (Consider using") . " " . $subnet . "/" . $subnetParse[1] . ")";
            } else {
                return true;
            }
        }
    ...
    public function verify_cidr($cidr) {
        $cidr = explode("/", $cidr);
        # verify network part
        if (strlen($cidr[0]) == 0 || strlen($cidr[1]) == 0) {
            return _("Invalid CIDR format!");
        }
        # verify network part
        if ($this->identify_address_format($cidr[0]) != "dotted") {
            return _("Invalid Network!");
        }
        # verify mask
        if (!is_numeric($cidr[1])) {
            return _("Invalid netmask");
        }
        if ($this->get_max_netmask($cidr[0]) < $cidr[1]) {
            return _("Invalid netmask");
        }
    }
    ...
    private function verify_IPv4_subnet_overlapping($subnet1, $subnet2) {
        ...
        if ($delta1 < $delta2) {
            ...
            if ($this->Net_IPv4->ipInNetwork(@$net1->network, $subnet2) || $this->Net_IPv4->ipInNetwork(@$net1->broadcast, $subnet2)) {
                return true;
            }
        } else {
            //check smaller nw and bc against bigger network
            if ($this->Net_IPv4->ipInNetwork(@$net2->network, $subnet1) || $this->Net_IPv4->ipInNetwork(@$net2->broadcast, $subnet1)) {
                return true;
            }
        }
        # do notoverlap
    ...
    private function verify_IPv6_subnet_overlapping($subnet1, $subnet2) {
        ...
        //verify
        if ($this->Net_IPv6->isInNetmask($subnet1, $subnet2)) {
            return true;
        }
    ...
    public function verify_subnet_resize($subnet, $mask, $subnetId, $vrfId, $masterSubnetId, $mask_old, $sectionId = 0) {
        ...
        # new mask must be > 8
        if ($mask < 8) {
            $this->Result->show("danger", _('New mask must be at least /8') . '!', true);
        }
        if (!is_numeric($mask)) {
            $this->Result->show("danger", _('Mask must be an integer') . '!', true);
            ;
        }
        ...
        if ($verify !== true) {
            $this->Result->show("danger", $verify, true);
        }

        # same mask - ignore
        if ($mask == $mask_old) {
            $this->Result->show("warning", _("New network is same as old network"), true);
        }
    ...
    private function verify_subnet_split($subnet_old, $number, $group, $strict) {
        ...
        switch ($number) {
            ...
            case "64": $mask_diff = 6;
                break;
            case "128": $mask_diff = 7;
                break;
            case "256": $mask_diff = 8;
                break;
            //otherwise die
    ...
    private function is_IPv4_subnet_inside_subnet($cidr1, $cidr2) {
        ...
        if (($this->Net_IPv4->ipInNetwork($cidr1->network, $cidr2)) && ($this->Net_IPv4->ipInNetwork($cidr1->broadcast, $cidr2))) {
            return true;
        } else {
            return false;
        }
    ...
    private function is_IPv6_subnet_inside_subnet($cidr1, $cidr2) {
        ...
        //check
        if ($this->Net_IPv6->isInNetmask($cidr1, $cidr2)) {
            return true;
        } else {
            return false;
        }
    ...
    private function find_unique_mastersubnetids() {
        try {
            $res = $this->Database->getObjectsQuery("select distinct(`masterSubnetId`) from `subnets` where `masterSubnetId` != '0' order by `masterSubnetId` asc;");
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    private function verify_subnet_id($id) {
        try {
            $res = $this->Database->getObjectQuery("select count(*) as `cnt` from `subnets` where `id` = ?;", array($id));
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    public function fetch_threshold_subnets($limit = 10) {
        try {
            $res = $this->Database->getObjectsQuery("select * from `subnets` where `threshold` != 0 and `threshold` is not null limit $limit;");
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    public function find_inactive_hosts($timelimit = 86400, $limit = 100) {
        ...
        // search
        try {
            $res = $this->Database->getObjectsQuery("select * from `ipaddresses` where `lastSeen` between ? and ? limit $limit;", array(date("Y-m-d H:i:s", strtotime(date("Y-m-d H:i:s")) - $timelimit), date("Y-m-d H:i:s", strtotime(date("Y-m-d H:i:s")) - (int) str_replace(";", "", strstr($this->settings->pingStatus, ";")))));
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    public function fetch_multicast_subnets() {
        ...
        // fetch
        try {
            $res = $this->Database->getObjectsQuery($query);
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
        ...
        # create
        if (sizeof($res) > 0 && sizeof($res2) > 0) {
            return $res2 + $res;
        } elseif (sizeof($res) > 0) {
            return $res;
        } elseif (sizeof($res2) > 0) {
            return $res2;
        } else {
            return false;
        }
    ...
    public function fetch_distinct_multicast_folders() {
        ...
        if (sizeof($res) > 0) {
            ...
            // fetch
            try {
                $res2 = $this->Database->getObjectsQuery($query);
            } catch (Exception $e) {
                $this->Result->show("danger", _("Error: ") . $e->getMessage());
                return false;
            }
    ...
    public function find_duplicate_multicast_mac($address_id, $mac) {
        ...
        // fetch
        try {
            $res = $this->Database->getObjectsQuery($query, array($mac, $address_id));
        } catch (Exception $e) {
            $this->Result->show("danger", _("Error: ") . $e->getMessage());
            return false;
        }
    ...
    private function multicast_address_exists($mac, $sectionId, $vlanId, $unique_required = "vlan", $address_id = 0) {
        ...
        if (sizeof($res) > 0) {
            ...
            foreach ($res as $line) {
                // overlap requirement
                if ($unique_required == "vlan" && $vlan_details->domainId == $line->domainId) {
                    return true;
                } elseif ($unique_required == "section" && $sectionId == $line->sectionId) {
                    return true;
                }
            }
        }
    ...
    public function parse_permissions($permissions) {
        switch ($permissions) {
            case 0: $r = _("No access");
                break;
            case 1: $r = _("Read");
                break;
            case 2: $r = _("Read / Write");
                break;
            case 3: $r = _("Read / Write / Admin");
                break;
            default: $r = _("error");
        }
        return $r;
    ...
    /**
     * Checks permission for user to add subnet
     *
     * 	we provide user details 
     *
     * @access public
     * @param object $user
     * 
     * @return void
     */
    public function subnet_check_permission($user) {
    ...
    public function print_subnets_menu($user, $section_subnets, $rootId = 0) {
        ...
        if (isset($_COOKIE['sstr'])) {
            $cookie = array_filter(explode("|", $_COOKIE['sstr']));
        } else {
            $cookie = array();
        }
        ...
        if (isset($_GET['section']))
            if (!is_numeric($_GET['section'])) {
                $this->Result->show("danger", _("Invalid ID"), true);
            }
        ...
        while ($loop && ( ( $option = each($children_subnets[$parent]) ) || ( $parent > $rootId ) )) {
            ...
            if (in_array($option['value']['id'], $allParents) ||
                    in_array($option['value']['id'], $cookie)) {
                $open = "open";
                $openf = "-open";
            } else {
                $open = "close";
            ...
            if ($option['value']['id'] == @$_GET['subnetId']) {
                if ($this->has_slaves(@$_GET['subnetId'])) {
                    $open = "open";
                    $openf = "-open";
                } else {
                    $open = "close";
            ...
            # for active class
            if ($_GET['page'] == "subnets" && ($option['value']['id'] == @$_GET['subnetId'])) {
                $active = "active";
                $leafClass = "";
            } else {
                $active = "";
            ...
            # override folder
            if ($option['value']['isFolder'] == 1 && ($option['value']['id'] == @$_GET['subnetId'])) {
                $open = "open";
                $openf = "-open";
                $active = "active";
            }
        ...
        foreach ($vlans as $item) {
            ...
            if ($subnets) {
                ...
                foreach ($subnets as $subnet) {
                    ...
                    if ($permission > 0) {
                        # for active class
                        if (isset($_GET['subnetId']) && ($subnet['id'] == $_GET['subnetId'])) {
                            $active = "active";
                            $leafClass = "";
                        } else {
                            $active = "";
    ...
    public function print_subnets_tools($user, $subnets, $custom_fields) {
        ...
        while ($loop && ( ( $option = each($children_subnets[$parent]) ) || ( $parent > $rootId ) )) {
            ...
            # vlan
            if (!array_key_exists($option['value']['vlanId'], $all_vlans)) {
                $vlan['number'] = "";
            } else {
...
//     			if ($count != $old_count) { $html[] = "</tbody><tbody>"; }
                ...
                //EditDate
                $editdate = $option['value']['editDate'];
                $html[] = "	<td class='hidden-xs hidden-sm'>$editdate</td>";
    ...
    public function print_mastersubnet_dropdown_menu($sectionId, $current_master = 0, $isFolder = false) {
        ...
        foreach ($section_subnets as $s) {
            // folders array
            if ($s->isFolder == 1) {
                $children_folders[$s->masterSubnetId][] = (array) $s;
            }
        ...
        if (sizeof(@$children_folders) > 0 || $isFolder) {
            ...
            while ($loopF && ( ( $option = each($children_folders[$parent]) ) || ( $parent > $rootId ) )) {
                ...
                if ($option === false) {
                    $parent = array_pop($parent_stack_folder);
                }
                # Has slave subnets
        ...
        if ($isFolder === false) {
            ...
            if (sizeof(@$children_subnets) > 0) {
                while ($loop && ( ( $option = each($children_subnets[$parent]) ) || ( $parent > $rootId ) )) {
                    ...
                    if ($option === false) {
                        $parent = array_pop($parent_stack_subnet);
                    }
                    # Has slave subnets
                    elseif (!empty($children_subnets[$option['value']['id']])) {
    ...
    public function search_available_subnets($subnetMasterId, $mask = false) {
        ...
        # mask check
        if (!is_numeric($mask)) {
            $this->Result->show("danger", _("Invalid Mask"), true);
        }
        if ($mask > 128 || $mask < 1) {
            $this->Result->show("danger", _("Invalid Mask"), true);
        }
    ...
    public function resolve_ripe_arin($subnet) {
        ...
        } elseif (in_array($subnet_check, $this->arin)) {
            return $this->query_arin($subnet);
        } else {
            return array("result" => "error", "error" => "$subnet Not RIPE or ARIN subnet");
        }
    }
    ...
    public function ripe_fetch_subnets($as) {
        ...
        if (!$ripe_connection) {
            ...
            $out = "";
            while (!feof($ripe_connection)) {
                $out .= fgets($ripe_connection);
            }
