...
//disabled links
    ...
    function showPopup(pClass, data, secondary) {
        ...
        if (secondary === true) {
            var oclass = "#popupOverlay2";
        }
        else {
            var oclass = "#popupOverlay";
        }
        // show overlay
        $(oclass).fadeIn('fast');
...
//default row count
    if (readCookie('table-page-size') == null) {
        ...
        def_size = readCookie('table-page-size');
    }
...
// hide submenus
    ...
    function update_subnet_structure_cookie(action, cid) {
        ...
        // defualt - if empty
        if (typeof s_cookie === 'undefined' || s_cookie == null || s_cookie.length === 0)
            s_cookie = "|";
        // add or replace
        if (action == "add") {
...
//submit ip address change
    $(document).on("click", "button#editIPAddressSubmit, .editIPSubmitDelete", function () {
        ...
        if ($(this).attr('id') == "editIPSubmitDelete") {
            postdata += "&deleteconfirm=yes&action=delete";
        }
        //replace delete if from visual
        if ($(this).attr('data-action') == "all-delete") {
            postdata += '&action-visual=delete';
        }
...
//ping check
    $(document).on("click", ".ping_ipaddress", function () {
        ...
        var subnetId = $(this).attr('data-subnetId');
        // new ip?
        if ($(this).hasClass("ping_ipaddress_new")) {
            id = $("input[name=ip_addr]").val();
        }
        //check
...
//show telnet port
    $(document).on('change', "table.table-scan select#type", function () {
        ...
        if (pingType == "scan-telnet") {
            $('tbody#telnetPorts').show();
        }
        else {
            $('tbody#telnetPorts').hide();
        }
    });
...
//import file script
    ...
    $(document).on("click", "input#csvImportYes", function () {
        ...
        var filetype = $('span.fname').html();
        //ignore errors
        if ($('input[name=ignoreErrors]').is(':checked')) {
            var ignoreError = "1";
        }
        else {
            var ignoreError = "0";
...
//submit request
    ...
    /*    request subnet for non-admins if locked or viewer
     *********************************************************/
...
//submit request
    $(document).on("click", "button#requestSubnetSubmit", function () {
        showSpinner();
        var request = $('form#requestSubnet').serialize();
        $.post('app/login/request_ip_result.php', request, function (data) {
            $('div#requestIPresult').html(data).slideDown('fast');
            reload_window(data);
        }).fail(function (jqxhr, textStatus, errorThrown) {
...
//reset input
    ...
    function search_execute(loc) {
        ...
        if (prettyLinks == "Yes") {
            ...
            window.location = base + "?page=tools&section=search&ip=" + ip;
        }
    }
//submit form - topmenu
...
//search export
    ...
    $('#hosts').submit(function () {
        ...
        var prettyLinks = $('#prettyLinks').html();
        if (prettyLinks == "Yes") {
            window.location = base + "tools/hosts/" + hostname;
        }
        else {
            window.location = base + "?page=tools&section=hosts&ip=" + hostname;
        }
        return false;
    });
...
//submit form
    $('form#cform').submit(function () {
        ...
        if (prettyLinks == "Yes") {
            ...
            window.location = "?page=tools&section=changelog&subnetId=" + filter + "&sPage=" + limit;
        }
        return false;
    });
    /* changePassRequired */
...
// clear logo
    ...
    $('select#mtype').change(function () {
        ...
        if (type === "localhost") {
            $('#mailsettingstbl tbody#smtp').hide();
        }
        else {
            $('#mailsettingstbl tbody#smtp').show();
        }
    });
    /* test mail */
...
// toggle notificaitons for user
    $(document).on("change", "form#usersEdit select[name=role]", function () {
        ...
        if (type == "Administrator") {
            $('tbody#user_notifications').show();
        }
        else {
            $('tbody#user_notifications').hide();
        }
    });
// generate random pass
    ...
    $('#instructionsForm').submit(function () {
        ...
        $.post('app/admin/instructions/edit-result.php', {instructions: instructions, csrf_cookie: csrf_cookie, id: id}, function (data) {
            ...
            if (data.search("alert-danger") == -1 && data.search("error") == -1) {
                $('div.instructionsResult').delay(2000).fadeOut('slow');
                hideSpinner();
            }
            else {
                hideSpinner();
...
//edit section result
    $(document).on("click", "#editSectionSubmit, .editSectionSubmitDelete", function () {
        ...
        //append deleteconfirm
        if ($(this).attr('id') == "editSectionSubmitDelete") {
            sectionData += "&deleteconfirm=yes";
        }
        ;
...
//load edit form
    $(document).on("click", ".editDomain", function () {
        ...
        if ($(this).hasClass('editDomain2')) {
            open_popup("700", "app/admin/powerDNS/domain-edit.php", {id: $(this).attr('data-id'), action: $(this).attr('data-action'), secondary: true}, true);
        }
        else {
...
// execute regeneration of the address object via ajax, reload the page to refresh the data
    $(document).on("click", "a.fw_autogen", function () {
        ...
        $.post('app/admin/firewall-zones/ajax.php', {subnetId: subnetId, IPId: IPId, dnsName: dnsName, action: action, operation: operation}).fail(function (jqxhr, textStatus, errorThrown) {
            showError(jqxhr.statusText + "<br>Status: " + textStatus + "<br>Error: " + errorThrown);
        });
...
//save edit subnet changes
    $(document).on("click", ".editSubnetSubmit, .editSubnetSubmitDelete", function () {
        ...
        //append deleteconfirm
        if ($(this).attr('id') == "editSubnetSubmitDelete") {
            subnetData += "&deleteconfirm=yes";
        }
        //load results
...
//delete folder
    $(document).on("click", ".editFolderSubmitDelete", function () {
        ...
        //append deleteconfirm
        if ($(this).attr('id') == "editFolderSubmitDelete") {
            postData += "&deleteconfirm=yes";
        }
        ;
        $.post('app/admin/subnets/edit-folder-result.php', postData, function (data) {
    ...
    $(document).on("click", "#editNatSubmit", function () {
        ...
        if (action !== "add") {
            ...
            $.post("app/admin/nat/edit-result.php", $('form#editNat').serialize(), function (data) {
                ...
                if (data.search("alert-danger") == -1 && data.search("error") == -1) {
                    setTimeout(function () {
                        open_popup("700", "app/admin/nat/edit.php", {id: $('div.new_nat_id').html(), action: "edit"});
                        hidePopup2();
                        parameter = null;
                    }, 1000);
                }
...
// remove item
    $(document).on("click", ".removeNatItem", function () {
        ...
        $.post("app/admin/nat/item-remove.php", {id: $(this).attr('data-id'), type: $(this).attr('data-type'), item_id: $(this).attr('data-item-id'), csrf_cookie: $('form#editNat input[name=csrf_cookie]').val()}, function (data) {
            ...
            if (data.search("alert-danger") == -1 && data.search("error") == -1) {
                setTimeout(function () {
                    open_popup("700", "app/admin/nat/edit.php", {id: id, action: "edit"});
                    hidePopup2();
                    parameter = null;
                }, 1000);
            }
...
//submit form
    $(document).on("click", "button.manageRequest", function () {
        ...
        // submit
        submit_popup_data(".manageRequestResult", "app/admin/subnetRequests/edit-result.php", postData);
    });
    /* ---- Share subnet ----- */
//load edit form
...
// add selected to db
    $(document).on("submit", "form#asImport", function () {
        ...
        $.post('app/admin/ripe-import/import-subnets.php', importData, function (data) {
            ...
            if (data.search("alert-danger") == -1 && data.search("error") == -1) {
                $('table.asImport').delay(1000).fadeOut('fast');
                hideSpinner();
            }
            else {
                hideSpinner();
    ...
    $('button#filterIPSave').click(function () {
        ...
        $.post('app/admin/filter-fields/filter-result.php', addata, function (data) {
            ...
            //reload after 2 seconds if succeeded!
            if (data.search("alert-danger") == -1 && data.search("error") == -1) {
                $('div.filterIPResult').delay(2000).fadeOut('slow');
                hideSpinner();
            }
            else {
                hideSpinner();
...
//Export Section
    $('button.dataExport').click(function () {
        var implemented = ["vrf", "vlan", "subnets", "ipaddr"];
        var popsize = {};
...
//Import Section
    $('button.dataImport').click(function () {
        var implemented = ["vrf", "vlan", "subnets", "recompute", "ipaddr"];
        var popsize = {};
        popsize["subnets"] = "max";
        popsize["ipaddr"] = "max";
        var dataType = $('select[name=dataType]').find(":selected").val();
...
// Paginate control of buttons
    ...
    $('.subnets').each(function () {
        ...
        if (id === "subnets") {
            ...
            $(this).append('<center><a class="btn btn-mini btn-success prev "><< prev</a>  <a class="btn btn-mini btn-info next ">next >></a></center>');
            ...
            $(this).find('.prev').click(function () {
                var first = $(this).parent().siblings('ul').children('li:visible:first');
