...
{section name=i loop=$dest_ports_untagged}
	<p>{$LBL_0_untag_success_port_id} <b>{$dest_ports_untagged[i]}</b> {$LBL_0_untag_success_switch} {$mySwitch->getName()} {$LBL_0_untag_success} <b>{$vlan_id}</b>.</p>
{/section}
